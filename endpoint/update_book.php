...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        // Update core fields
        $stmt = $conn->prepare("UPDATE `tbl_book` SET `book_title` = ?, `book_category` = ?, `book_author` = ?, `book_abstract` = ?, `book_text` = ? WHERE `tbl_book_id` = ?");
        $stmt->execute([$bookTitle, $bookCategory, $bookAuthor, $bookAbstract, $bookText, $bookID]);

        // Handle image upload
        if (!empty($_FILES['book_image']['name'])) {
            $imageDir = __DIR__ . '/../image';
            $imageName = uploadFile('book_image', $imageDir, ['jpg','jpeg','png','gif']);
            $stmt = $conn->prepare("UPDATE `tbl_book` SET `book_image` = ? WHERE `tbl_book_id` = ?");
            $stmt->execute([$imageName, $bookID]);
        }
